...
/*
 * Планы продаж: факт **того же месяца** с амбицией +5% и разбросом 13%.
 *
 * Комментарий здесь раньше обещал «от факта прошлого года» — код так
 * никогда не делал, и это ровно тот класс расхождения, который гейты
 * не видят в принципе (они считают строки, а не читают, чем те объяснены).
 * Названо честно, потому что из этого следует граница: **план тут знает
 * будущее**, его отклонение от факта — шум вокруг +5%, а не результат
 * планирования. Считать по нему точность прогноза нельзя; для этого
 * заведён fact_forecast_snapshot, где снимок сделан заранее.
 */
const targets = [];
{
  let tId = 0;
...
// ------------------------------------------- снимки прогноза спроса (S&OP)

/**
 * `fact_forecast_snapshot` — что планировщик спроса думал о месяце заранее.
 *
 * **Собственный поток случайных чисел, и это не стилистика, а единственный
 * безопасный способ добавить таблицу в готовый датасет.** ГПСЧ здесь один
 * на весь генератор и детерминирован: любой лишний вызов `rnd()`, вставленный
 * выше по файлу, сдвигает всю последовательность после себя — то есть молча
 * переписывает эталонные ответы всех заданий, которые считаются по этим
 * данным. Отдельный `mulberry32` с другим зерном и генерация в самом конце
 * оставляют основной поток нетронутым; проверяется это не рассуждением,
 * а сверкой контрольных сумм прочих таблиц до и после правки.
 *
 * Почему не годится `fact_target` (вопрос возникает первым): план там
 * строится от факта **того же месяца** (`actual × 1.05 ± 13%`), то есть
 * знает будущее. Ошибку такого «прогноза» считать бессмысленно — она
 * по построению чистый шум вокруг +5%.
 *
 * Гранулярность — месяц × SKU × горизонт. Именно SKU: месячные объёмы
 * разбегаются от 1 до ~5000 штук, и **на этом разбросе MAPE и WMAPE
 * расходятся**, а ради этого расхождения таблица и заводится. На уровне
 * бренда или дивизиона объёмы одного порядка, и взвешивание перестаёт
 * что-либо менять.
 *
 * В ошибку намеренно заложены пять явлений — каждое проверяется гейтом
 * в verify-dataset.mjs и годится в задание:
 *
 *  1. **Точность падает с горизонтом.** Чем раньше сделан прогноз, тем он
 *     хуже: базовый факт цикла S&OP.
 *  2. **Устойчивый перепрогноз «Nettora».** Бренд теряет дистрибуцию
 *     (сюжет уже в данных), а планировщик ждёт возврата — смещение вверх
 *     месяц за месяцем. Даёт разведение смещения и точности: ошибки,
 *     которые не гасят друг друга, — другой диагноз, чем те, что гасят.
 *  3. **Новинка «Vitanor Forte» — худшая ошибка датасета.** Истории нет,
 *     прогнозировать не из чего; первые месяцы после запуска мажут вдвое.
 *  4. **Промо-месяцы хуже обычных.** Прогноз делается до подтверждения
 *     акции, поэтому всплеск спроса в него не попадает.
 *  5. **Мелкие SKU: огромный процент ошибки при ничтожном вкладе в итог.**
 *     Ровно то, ради чего WMAPE и придуман.
 *
 * Чего таблица не знает — и это граница содержания, а не список задач
 * на будущее: нет версий прогноза, нет автора, нет разделения на
 * статистический и согласованный. Последнее — сознательный отказ:
 * наивный прогноз («в этом месяце будет как в прошлом») считается из
 * `fact_sellout` и без второй колонки, то есть forecast value added
 * получается бесплатно, и часть SKU наш прогноз наивному проигрывает.
 */
const forecasts = [];
{
  const frnd = mulberry32(20260822);
  const fJitter = (mean, spread) => mean * (1 + (frnd() + frnd() - 1) * spread);

  // факт по месяцу и SKU — то, что прогноз пытается угадать
  const actualByMonthProduct = new Map();
  for (const r of sellout) {
    const key = `${r.week_start.slice(0, 7)}-01|${r.product_id}`;
    actualByMonthProduct.set(key, (actualByMonthProduct.get(key) ?? 0) + r.units);
  }

  /** Был ли у бренда промо-старт в этом месяце: акцию прогноз не ждёт. */
  const promoMonths = new Set();
  for (const p of promos) promoMonths.add(`${p.start_date.slice(0, 7)}|${p.brand}`);

  let fId = 0;
  for (const mth of months) {
    for (const p of products) {
      const actual = actualByMonthProduct.get(`${mth}|${p.product_id}`);
      if (actual === undefined) continue; // месяца без продаж прогноз не касается

      const monthsSinceLaunch =
        (parseISO(mth) - parseISO(p.launch_date.slice(0, 7) + '-01')) / (365.25 * DAY) * 12;

      for (const lag of [1, 2, 3]) {
        const snapshot = iso(new Date(Date.UTC(
          parseISO(mth).getUTCFullYear(),
          parseISO(mth).getUTCMonth() - lag,
          1,
        )));
        if (snapshot < iso(START)) continue; // до начала периода прогнозов нет

        /*
         * Разброс растёт с горизонтом. Числа подобраны замером, а не взяты
         * из головы: `jitter` даёт треугольное распределение, у которого
         * средняя по модулю ≈ spread/3, то есть WMAPE выходит примерно
         * 15 / 21 / 28% на горизонтах 1 / 2 / 3. Это порядок настоящей
         * точности на паре месяц-SKU в FMCG, и, что важнее, три уровня
         * различимы глазом в результате запроса.
         */
        let spread = 0.24 + lag * 0.20;
        // Систематическое смещение — вверх у падающего бренда, около нуля у прочих.
        let bias = p.losingDistribution ? 0.10 + lag * 0.045 : 0;

        /*
         * Новинка и промо портят прогноз только на дальних горизонтах,
         * и это не упрощение, а суть цикла S&OP: за месяц до начала
         * промо-календарь уже подтверждён, а у новинки набралась первая
         * история. Ошибиться планировщик может и на лаге 1, но
         * систематически недооценивать — уже нет.
         */
        /*
         * Окно запуска — шесть месяцев, а не три, и это поправка по замеру.
         * У единственной настоящей новинки датасета («Vitanor Forte x30»,
         * сентябрь 2025) первые месяцы дают 1, 9 и 19 штук: ошибка там
         * ничтожна в штуках и потому не видна ни в WMAPE, ни в задании.
         * Раскатка выводит SKU на осмысленный объём к четвёртому-шестому
         * месяцу (41 → 135 штук) — именно туда и должна приходиться
         * недооценка, иначе явление есть в замысле и отсутствует в данных.
         */
        if (lag >= 2 && monthsSinceLaunch >= 0 && monthsSinceLaunch < 6) {
          spread += 0.70 - monthsSinceLaunch * 0.09;
          bias -= 0.30; // запуск недооценивают: осторожничают, пока нет истории
        }
        if (lag >= 2 && promoMonths.has(`${mth.slice(0, 7)}|${p.brand}`)) {
          spread += 0.16;
          bias -= 0.13;
        }

        /*
         * Хвост мелких SKU шумит в процентах сильно, а в штуках незаметно —
         * ради этого расхождения таблица и заведена. Порог взят из данных,
         * а не назначен: пары с объёмом ниже 200 штук — это четверть строк
         * и 3.4% всего объёма (замер по fact_sellout). Прежний порог в 40
         * штук покрывал 13 строк из 1390, и MAPE с WMAPE не расходились
         * вовсе — дефект замысла, пойманный только замером.
         */
        if (actual < 200) spread *= 1 + ((200 - actual) / 200) * 1.8;

        const forecast = Math.max(1, Math.round(fJitter(actual * (1 + bias), spread)));
        forecasts.push({
          forecast_id: ++fId,
...
const SCHEMA = [
  ...
  {
    table: 'fact_forecast_snapshot',
    title: { ru: 'Снимки прогноза спроса', en: 'Demand forecast snapshots' },
    grain: {
      ru: 'месяц × SKU × горизонт прогноза',
      en: 'month × SKU × forecast horizon',
    },
    note: {
      ru: 'Что планировщик спроса думал о месяце заранее — за один, два и три месяца. Строка не меняется задним числом: это снимок на момент, когда прогноз был сделан. Факт для сравнения берётся из fact_sellout.',
      en: 'What the demand planner expected for a month one, two and three months ahead. A row is never rewritten afterwards: it is a snapshot taken when the forecast was made. The actuals to compare against live in fact_sellout.',
    },
    ddl: `CREATE TABLE fact_forecast_snapshot (
      forecast_id INTEGER PRIMARY KEY, snapshot_month TEXT, month_start TEXT,
      lag_months INTEGER, product_id INTEGER, forecast_units INTEGER)`,
    cols: {
      forecast_id: { ru: 'Ключ строки', en: 'Row key' },
      snapshot_month: {
        ru: 'Месяц, в котором прогноз сделан',
        en: 'Month when the forecast was made',
      },
      month_start: { ru: 'Месяц, на который прогноз сделан', en: 'Month the forecast is for' },
      lag_months: {
        ru: 'Горизонт: за сколько месяцев до начала месяца сделан прогноз (1, 2 или 3)',
        en: 'Horizon: how many months ahead the forecast was made (1, 2 or 3)',
      },
      product_id: { ru: 'FK → dim_product', en: 'FK → dim_product' },
      forecast_units: { ru: 'Прогноз спроса, шт', en: 'Forecast demand, units' },
    },
    rows: forecasts,
  },
  {
