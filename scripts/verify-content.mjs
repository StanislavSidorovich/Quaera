...
/**
 * Исполняется ли код этого трека по-настоящему.
 *
 * Держится в одном месте, потому что от этого зависят три разные вещи:
 * какие режимы заданию разрешены, запускается ли эталон и требуется ли
 * от пропусков быть содержательными. Зеркалит `runsCode` у исполнителя
 * (src/engine/executors.ts) — если реестр там изменится, эта строка обязана
 * измениться вместе с ним.
 */
const runsCodeTrack = (track) => track === 'sql' || track === 'python';
...
for (const packId of packs) {
  ...
  for (const t of pack.tasks) {
    ...
    /*
     * Треки без исполнителя кода.
     *
     * `domain` остаётся только в predict: там задания — бизнес-ситуации,
     * писать нечего в принципе.
     *
     * `model` умеет ещё и fill. Исполнителя DAX нет и не будет (ROADMAP §6),
     * но fill его и не требует: ответ сверяется с эталоном как текст
     * (src/engine/textGrade.ts), а шаблон вокруг пропусков задан заданием.
     * Режим `write` тут по-прежнему запрещён — свободную формулу сверить
     * текстом нельзя, у неё слишком много верных написаний.
     */
    ...
    if (t.mode === 'fill') {
      if (!t.template || !t.blanks) {
        ...
        /*
         * Пропуск обязан быть решением, а не попаданием по клавише.
         *
         * Там, где код исполняется, пустяковый пропуск ловится задачей
         * целиком: ответ всё равно проверяется запуском. Там, где сверка
         * идёт текстом, этой страховки нет — и `fill` вырождается в угадайку
         * ровно тогда, когда в пропуске стоит одиночный символ вроде `)`
         * или цифра. Порог намеренно низкий: он отсекает вырождение,
         * а не задаёт стиль.
         */
        if (!runsCodeTrack(pack.track)) {
          for (const [i, b] of t.blanks.entries()) {
            if (String(b).trim().length < 2) {
    ...
    // Сверка текстом — единственная проверка этого задания, поэтому здесь
    // маршрут и заканчивается: исполнять нечего.
    if (!runsCodeTrack(pack.track)) continue;
