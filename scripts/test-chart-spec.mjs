...
try {
  ...
  const withRank = chartSpec(
    preview(
      ...
      [
        ['Vitanor', 'Омега-3', 1754553, 1],
        ['Vitanor', 'D3', 1259055, 2],
        ['Fruvia', 'Апельсин', 900000, 1],
      ]
    )
  );
  ...
  assertEq('подпись — название, а не номер', withId.labels[0], 'Moscow');
  ...
  const mixedUnits = chartSpec(
    preview(
      ...
      [
        ['Aqualis', 8012489, 20.9],
        ['Milvara', 5673447, 14.8],
        ['Vitanor', 3146133, 8.2],
      ]
    )
  );
