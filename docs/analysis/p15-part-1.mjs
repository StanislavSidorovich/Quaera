...
// ---------------------------------------------------------------------------
/**
 * id дней. Где смысл дня пережил перестройку, id остаётся прежним — позиция
 * хранится id дня (App.tsx STORY_MISSION_STORAGE_KEY), и стоявший на нём продолжит
 * с того же по смыслу дня без карты переходов:
 ...
 * Новые дни — с префиксом p1-. Карта STORY_MISSION_RENAMED не нужна: ни один старый id
 * не исчезает. Цена этого — стоявший на day-4-join пропустит новые Пт/Сб w1 и Пн w1b;
 * это те же числа, что у него уже были (sql-097, sql-099), только собранные по Nettora,
 * и папка дела перескажет их одной строкой found — не наказание, а сокращение.
 * Чужих прохождений в Supabase ноль (замер 35-го захода), ключ кампании локальный.
 */
export const LAYOUT = {
  w1: [
 ...
 *
 * 5. **Миграция прогресса не нужна** — см. LAYOUT: старые id дней не исчезают.
 *    Нынешняя неделя w1 распадается на w1 + w1b, дни w2…w5 и их id не трогаются.
 */
