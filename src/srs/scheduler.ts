...
/**
 * Сколько времени после прошлого успеха считается тем же присестом. Полсуток,
 * а не сутки: вечер одного дня кампании и утро следующего — это уже сон между
 * ними, то есть настоящее повторение, и расти ему положено.
 */
const SAME_SITTING_MS = 12 * 3600 * 1000;
const MIN_EASE = 1.3;
const MAX_EASE = 2.8;
...
export function review(state: SkillState, grade: Grade, now = new Date()): SkillState {
  ...
  /*
   * Успех в тот же присест интервал не растит. Интервал — это «сколько
   * человек помнит без повторения», а четыре задания на один приём за полчаса
   * памяти не проверяют: второе решается по следам первого. До правки каждое
   * такое задание шло за отдельное повторение с отдельным интервалом, и день 6
   * кампании (четыре задания на LEFT JOIN подряд) отправлял приём на 39 дней
   * вперёд — 2 → 5 → 14 → 39, — а неделя 4 отправляла даты и ряды на 180.
   * То есть приёмы самых трудных дней пропадали из повторений на месяцы.
   *
   * Провал по-прежнему сбрасывает (ветка выше), и первый успех после провала
   * по-прежнему назначает первый интервал: при `intervalDays === 0` навык
   * ещё не держится ни дня, расти нечему. Лёгкость в присест тоже не трогаем —
   * иначе серия подряд раскачала бы её так же, как раскачивала интервал.
   */
  if (
    state.intervalDays > 0 &&
    state.lastReviewedAt !== null &&
    now.getTime() - Date.parse(state.lastReviewedAt) < SAME_SITTING_MS
  ) {
    return next;
  }
