...
const ru: StoryCampaign = {
  ...
  missions: [
     ...
     */
    {
      ...
      hook: [
        'Прежде чем искать виноватого снаружи, стоит закрыть версии внутри. Ито-сан в коридоре называл две: ушли торговые и подняли цены. Про цены мы ещё не смотрели вовсе.',
      ...
      steps: [
        {
          ...
          intro: {
            paras: [
              'Разминка на знакомом. Соединение со справочником, группировка по выражению, ROUND(AVG(...)) — всё это уже было. Нового в этом запросе нет ничего, есть только вопрос.',
              'Год берётся так же, как месяц в форме года, только короче: substr(fp.month_start, 1, 4). Соединение уже стоит в заготовке, осталось написать, что выбрать, кого отобрать и по чему свернуть.',
            ],
          },
          after: {
            from: 'Ваш руководитель',
      ...
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              ...
              'Нового в этом запросе нет ничего: WITH и соединение двух выражений из среды, деление через 1.0 оттуда же, JOIN, BETWEEN и ORDER BY DESC — давно знакомые. Собери и посмотри на числа: тебе нужен не первый в списке, а форма всего списка.',
            ],
          },
          after: {
            from: 'Ваш руководитель',
     ...
     */
    {
      ...
      hook: [
        'Начинал ты с того, что не мог достать список товаров. Сегодня ты закрыл версии числами, назвал границу того, что данные знают, не выдал догадку за вывод — и отдал в поле список, по которому можно работать. Это и есть работа, за которую платят.',
...
const en: StoryCampaign = {
  ...
  missions: [
    ...
    },
    {
      ...
      hook: [
        'Before hunting for a culprit outside, the versions inside deserve closing. Ito named two in the corridor: the reps who left and the price that went up. Price we have not looked at at all.',
...

    {
      ...
      messages: [
        {
          from: 'Your manager',
          text: '"The price version checks out faster than any other, so we start there. First the list price: work out Nettora\'s average price by year, all of it familiar. If the list price barely moved, the question is not the list price but the discount. If the brand spent two years on promotion and stood on it, the end of a promotion explains the fall with no competitor at all."',
        },
        {
          from: 'Aoki-san, sales director',
      ...
      steps: [
        {
          ...
          intro: {
            paras: [
              'A warm-up on familiar ground. The join to a dimension, grouping by an expression, ROUND(AVG(...)): you have seen them all. Nothing in this query is new; only the question is.',
              'The year is taken the way the month was for the shape of the year, just shorter: substr(fp.month_start, 1, 4). The join is already in the starter; what is left is what to select, what to filter and what to group by.',
            ],
          },
          after: {
            from: 'Your manager',
      ...
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              ...
              'Nothing in this query is new: WITH and the join of two expressions come from Wednesday, division through 1.0 from there as well, and JOIN, BETWEEN and ORDER BY DESC you have known for a while. Assemble it and look at the numbers. What you need is not the first line but the shape of the whole list.',
            ],
          },
          after: {
            from: 'Your manager',
...

    {
      ...
      hook: [
        'You started out unable to pull a list of products. Today you closed versions with numbers, named the boundary of what the data knows, did not pass a guess off as a conclusion, and handed the field a list it can work from. That is the work people are paid for.',
