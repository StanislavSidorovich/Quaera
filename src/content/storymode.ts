...
const ru: StoryCampaign = {
  ...
  missions: [
     ...
     */
    {
      ...
      hook: [
        'Первая неделя закрыта, и её вопрос тоже: Nettora упала по-настоящему, и не из-за рынка.',
     ...
     */
    {
      ...
      messages: [
        ...
        {
          from: 'Ваш руководитель',
          text: '«Прошлую неделю ты закрыл ответом: упали по-настоящему. Теперь вопрос про причину, и Ито-сан даёт проверяемую версию. Штуки — это точки, умноженные на продажи на одну точку. Начни с точек: в скольких местах бренд вообще продаётся?»',
        },
      ],
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              'Штуки складываются из двух вещей: в скольких точках бренд стоит и сколько уходит через каждую. Первое — полка. В продажах точка — это customer_id, а число разных точек считает COUNT(DISTINCT customer_id): тот же приём, что для брендов на прошлой неделе, только колонка другая.',
     ...
     */
    {
      ...
      hook: [
        'Вторник второй недели закрыт: теперь ты называешь бренд по имени, а не по номерам.',
        ...
        },
        {
          ...
          intro: {
            ...
            paras: [
              ...
              'HAVING в задании уже написан. Соединение и группировку по бренду допиши сам: первое ты печатал вчера, второе — на прошлой неделе.',
            ],
          },
          after: {
            from: 'Аоки-сан, директор по продажам',
     ...
     */
    {
      ...
      reflection: [
        ...
        'Вот теперь это ответ, а не уточнённый вопрос. У него есть не только виновник, но и владелец: не маркетинг и не цена, а переговоры с сетями и работа полевой команды. Заметь, что изменилось за неделю — две недели назад ты не мог достать даже список товаров, сегодня называешь причину падения и адресата.',
      ],
      hook: [
     ...
     */
    {
      ...
      steps: [
        {
          ...
          intro: {
            title: 'Свой запрос с чистого листа',
            paras: [
              'Вопрос субботы: в скольких разных точках продавался каждый бренд с начала 2026 года. Части все знакомые: соединение продаж со справочником товаров и период без верхней границы — со вторника, счёт разных точек — с понедельника, группировка и сортировка по убыванию — с первой недели.',
              'Порядок частей в запросе тоже знакомый: что показать, откуда, какие строки, по чему свернуть, как упорядочить. Подсказки на месте, но сначала попробуй без них — ради этого суббота и есть.',
      ...
      hook: [
        'Вторая неделя закрыта, и её вопрос тоже: Nettora потеряла полку, а не спрос, и потеряла одна.',
...

    {
      ...
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              ...
              'И с тем же риском, что разбирали в SQL на второй неделе: если ключ повторяется с обеих сторон, каждая строка слева приклеится ко всем совпавшим справа. Ниже продажи соединяют с акциями по бренду. Прочитайте и скажите, сколько строк получится.',
            ],
          },
        },
        {
      ...
      reflection: [
        ...
        'Для Setouchi это значит простую вещь. Отношение отгрузок к продажам — это две свёрнутые таблицы, соединённые по дистрибьютору, ровно как запрос с двумя WITH на второй неделе. Теперь его можно собрать кодом и пересчитать за любой квартал одной правкой.',
      ],
      hook: [
      ...
      steps: [
        {
          ...
          after: {
            from: 'Ваш руководитель',
            text: '«2.44, как в пятницу второй недели, у одиннадцати остальных — 1.04 и 1.05. Код сошёлся с запросом, значит, мерке можно верить. Теперь январь–март.»',
          },
        },
        {
          taskId: 'py-056',
...

    {
      ...
      steps: [
        ...
        },
        {
          ...
          after: {
            from: 'Мори-сан, КАМ по дистрибьюторам',
            text: '«Пять месяцев продаж на складе, а я всем говорил, что всё вернулось. Две недели назад тебя посадили за прайс-лист, а в понедельник я иду к Setouchi — и иду с этой таблицей, а не со своей перепиской.»',
          },
        },
      ],
      reflection: [
...
const en: StoryCampaign = {
  ...
  missions: [
...

    {
      ...
      hook: [
        'Week one is closed, and so is its question: Nettora really fell, and not because of the market.',
...

    {
      ...
      messages: [
        ...
        {
          from: 'Your manager',
          text: '"Last week you closed with an answer: it really fell. Now the question is the cause, and Ito-san gives a testable version. Units are outlets multiplied by sales per outlet. Start with the outlets: in how many places is the brand sold at all?"',
        },
      ],
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              'Units are made of two things: in how many outlets the brand stands and how much moves through each. The first is the shelf. In sales an outlet is a customer_id, and the number of distinct outlets is COUNT(DISTINCT customer_id): the same move as for brands last week, only on a different column.',
...

    {
      ...
      hook: [
        'Tuesday of week two is closed: now you name a brand by name, not by numbers.',
        ...
        },
        {
          ...
          intro: {
            ...
            paras: [
              ...
              'HAVING is already written in the task. The join and the grouping by brand are yours: the first you typed yesterday, the second last week.',
            ],
          },
          after: {
            from: 'Aoki-san, sales director',
...

    {
      ...
      reflection: [
        ...
        'Now this is an answer rather than a sharper question. It has a culprit and it has an owner: not marketing and not price, but negotiations with the chains and the work of the field team. Notice what changed in a week. Two weeks ago you could not even pull a list of products; today you name the cause of a decline and the person who owns it.',
      ],
      hook: [
...

    {
      ...
      steps: [
        {
          ...
          intro: {
            title: 'Your own query from a blank page',
            paras: [
              'Saturday\'s question: in how many distinct outlets did each brand sell since the start of 2026. Every part is familiar: joining sales to the product directory and a period with no upper bound come from Tuesday, counting distinct outlets from Monday, grouping and sorting in descending order from week one.',
              'The order of the parts is familiar too: what to show, where from, which rows, what to fold by, how to sort. The hints are in place, but try without them first; that is what Saturday is for.',
      ...
      hook: [
        'Week two is closed, and so is its question: Nettora lost its shelf, not its demand, and it lost it alone.',
...

    {
      ...
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              ...
              'And with the same risk you took apart in SQL in week two: if the key repeats on both sides, every row on the left gets glued to every matching row on the right. Below, sales are joined to promotions by brand. Read it and say how many rows come out.',
            ],
          },
        },
        {
      ...
      reflection: [
        ...
        'For Setouchi this means something simple. The ratio of shipments to sales is two collapsed tables joined on the distributor, exactly like the query with two WITH blocks in week two. Now it can be assembled in code and recomputed for any quarter with a single edit.',
      ],
      hook: [
      ...
      steps: [
        {
          ...
          intro: {
            ...
            paras: [
              'The ratio is assembled the same way as the query with two WITH blocks in week two: each fact is first collapsed to one row per distributor, and only then are the collapsed tables joined. Shipments collapse straight away, since the distributor is in them. Sales do not have it, so each sale first gets its outlet attached, with a join on customer_id, where the key is unique on the right.',
          ...
          after: {
            from: 'Your manager',
            text: '"2.44, as on the Friday of week two, with the other eleven at 1.04 and 1.05. The code agrees with the query, so the yardstick can be trusted. Now January to March."',
          },
        },
        {
          taskId: 'py-056',
...

    {
      ...
      steps: [
        ...
        },
        {
          ...
          after: {
            from: 'Mori-san, key account manager for distributors',
            text: '"Five months of sales in the warehouse, and I kept telling everyone it was all back to normal. Two weeks ago they sat you down with the price list; on Monday I go to Setouchi, and I go with this table, not with my correspondence."',
          },
        },
      ],
      reflection: [
