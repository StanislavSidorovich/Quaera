 ...
 * (где WelcomeHero держит её в одной карточке с абзацем-питчем) сразу
 * под ней идёт следующая карточка, а не ещё один ряд ссылок, поэтому
...
function Home({
  ...
  return (
    <>
      ...
      {/*
       * Обоснование после решения: «С чего начать», три довода и цепочка —
       * тому, кто уже увидел карточки треков и хочет понять, чему верить
       * и с чего конкретно начинать. См. комментарий у WelcomeHero выше.
       */}
      {isNewUser && (
        <div className="card welcome-hero">
          <WelcomeProofBody onOnboarding={onOpenOnboarding} />
        </div>
      )}
