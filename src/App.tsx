...
/**
 * Какие шаги цепочки `welcome.chainSteps` закрывает трек — индексами, а не
 * словами.
 *
 * Словами не выйдет: подпись трека говорит «вопрос», а шаг цепочки называется
 * «бизнес-вопрос», и сопоставление по тексту разошлось бы на первой же правке
 * формулировки, да ещё по-разному в двух локалях. Индексы от локали не зависят
 * вовсе — это структура пути, а не его название.
 *
 * У `domain` два несмежных участка: трек открывает цепочку постановкой вопроса
 * и закрывает её подачей вывода (см. `tracks.chainStage`, где то же сказано
 * строкой).
 */
const CHAIN_STAGE_STEPS: Record<Track, number[]> = {
  domain: [0, 1, 2, 6, 7],
  sql: [3],
  python: [4],
  model: [5],
...
export default function App() {
  ...
  return (
    <div className={`app${fontSize !== 'md' ? ` font-${fontSize}` : ''}`}>
      ...
      <div className="shell">
        {/*
         * `topbar-home` — модификатор главной, а не всех экранов: только там
        ...
        >
                    ...
                    : screen.name === 'about'
                      ...
                      : screen.name === 'trackIntro'
                        ? /*
                          ...
                          null
                        : /*
                           * Главная: только имя трека. Серия дней ушла отсюда
                           * в строку-разделитель под полоской треков
                           * (.topbar-streak ниже) — подпись заголовка держала
                           * два разных сообщения подряд («где я» и «сколько
                           * дней подряд»), и на 360px они переносились
                           * на вторую строку, растягивая первую строку шапки.
                           */
                          t.tracks.names[activeTrack]}
            </span>
          </h1>
          ...
          {/*
           * Серия дней — не в подписи заголовка, а строкой-разделителем
           * по нижней границе шапки, с разрывом под текст.
           *
           * Довод не косметический. В подписи она стояла хвостом за именем
           * трека («SQL для аналитика · серия 1 дн.»), то есть два разных
           * по смыслу сообщения делили одну строку и на 360px переносились,
           * поднимая первую строку шапки до двух строк текста. Здесь строке
           * ничего не мешает: она занимает ту вертикаль, которую граница
           * шапки занимала и так, — 1px линии превращается в 16px строки,
           * а заголовок худеет ровно на строку.
           *
           * Отдельным элементом, а не ::after на шапке: содержимое
           * переводится и меняется числом, а псевдоэлемент с content
 ...
 */
/**
 * Карта кампании: ряд на неделю, клетка на день — ряды и клетки берутся
 * из самой кампании, а не зашиты. С 2026-09-14 рядов пять: дело Setouchi
 * разложено на две недели (перебалансировка недель 4–5). Фраза абзаца
 * «дел четыре» при этом осталась верной — дел по-прежнему четыре, просто
 * последнее идёт две недели, и карта показывает это двумя рядами одного цвета.
 *
...
function About({
  ...
  return (
    <>
      ...
      {/*
       * «Почему сделано так» — два решения и цена каждого. Во всю ширину
       * и вне about-columns: колонки ниже балансируются по высоте парой
       * «Как устроено» / «Приватность + Автор», и третий блок эту пару
       * ломает (тот же довод, что у карточки установки ниже).
       *
       * Место выбрано по порядку чтения: «Структура» отвечает, что внутри,
       * эта карточка — почему именно так, и только за ней идёт «Как это
       * устроено» с механикой. Решение перед механикой, а не после неё:
       * SRS и три режима ввода объясняются проще, когда уже сказано, что
       * это тренажёр, а не курс.
       */}
      <div className="card">
        <h2>{t.about.whyTitle}</h2>
        {/* Две колонки на широком экране — см. .why-columns: абзац ограничен
            68ch, и во всю ширину карточка была наполовину пуста. */}
        <div className="why-columns">
          <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6 }}>{t.about.whyGenreBody}</p>
          <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6 }}>{t.about.whyAccountBody}</p>
        </div>
      </div>
...
function TrackIntroScreen({
  ...
  /*
   * Пример — первое задание пака, а не выбранный руками.
   *
   * Выбранное руками пришлось бы держать четырьмя парами строк на двух
   * локалях и следить, чтобы оно не разошлось с содержимым пака: то же
   * расхождение «число набрано рядом с числом, которое считается», которое
   * этот проект уже ловил дважды. Первое задание годится по построению —
   * порядок в паке идёт от простого, и человек увидит ровно то, с чего
   * начнёт, если нажмёт «Начать занятие».
   */
  const sample = pack?.tasks[0];

  return (
    <>
      {/*
       * Шапка — вход для глаза. До неё экран открывался словами «Что это»,
       * то есть самым отвлечённым из пяти блоков, и зацепиться на нём было
       * не за что: пять карточек сплошной прозы читаются как страница
       * документации.
       *
       * Три вещи, и ни одна не заводит нового контента: участок цепочки
       * (та же строка, что под названием трека на главной), два числа
       * из самого пака и первое задание — «вот как это выглядит» раньше,
       * чем «вот что это такое».
       */}
      <div className="card track-intro-head">
        {/*
         * Вся цепочка с подсвеченным участком, а не только сам участок.
         *
         * Участком строкой (`tracks.chainStage`) шапка и открывалась, но
         * у SQL он состоит из одного слова «данные», и над карточкой оно
         * висело без всякого контекста: на главной та же строка стоит под
         * названием трека и рядом с описанием, а здесь читать её было не
         * с чем. Целиком путь отвечает на вопрос, которого подпись даже
         * не задаёт, — где этот трек среди остальных.
         */}
        <div className="chain-steps track-intro-chain">
          {t.welcome.chainSteps.map((step, i) => (
            <span key={step} className="chain-step-wrap">
              <span className="pill chain-step" data-on={CHAIN_STAGE_STEPS[track].includes(i) || undefined}>
                {step}
              </span>
              {i < t.welcome.chainSteps.length - 1 && <span className="chain-arrow">→</span>}
            </span>
        ...
        {/*
         * Пример и числа рядом, а не друг под другом: абзац примера ограничен
         * 68 символами ради читаемости, и на широком экране справа от него
         * оставалось бы полкарточки пустоты. Числа занимают её собой и не
         * растягиваются — на узком экране блоки просто встают в столбик.
         */}
        <div className="track-intro-head-body">
          {sample && (
            <div className="track-intro-sample">
              <p className="track-intro-sample-label">{t.trackIntro.sampleTitle}</p>
              <p className="track-intro-sample-brief">{sample.brief}</p>
              <p className="track-intro-sample-goal">{sample.goal}</p>
            </div>
          )}
          <div className="track-intro-stats">
            <div>
              <div className="big">{pack.tasks.length}</div>
              <div className="muted">{t.about.tasksStatLabel}</div>
            </div>
            <div>
              <div className="big">{pack.skills.length}</div>
              <div className="muted">{t.about.skillsStatLabel}</div>
            </div>
          </div>
        </div>
