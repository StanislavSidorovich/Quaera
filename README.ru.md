...
**Четыре трека — 294 задания и 73 навыка.** Наполнены целиком, вычитаны
и переведены на английский: интерфейс, задания, карточки приёмов.

| Трек | Заданий | Навыков | Как проверяется ответ |
| --- | --- | --- | --- |
| SQL для аналитика | 91 | 20 | Запрос выполняется в SQLite (sql.js) |
...
| pandas для аналитика | 47 | 14 | Код выполняется в Python (Pyodide) |
...
ранний прототип. Рядом с ним идёт отдельная сюжетная линия через каждый
трек, прозой закрытая для всех четырёх.
