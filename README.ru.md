...
Код: [Apache-2.0](LICENSE) · Учебный контент: [CC BY-NC-SA 4.0](LICENSE-CONTENT)
...
**Четыре трека — 342 задания и 73 навыка.** Наполнены целиком, вычитаны
и переведены на английский: интерфейс, задания, карточки приёмов.

| Трек | Заданий | Навыков | Как проверяется ответ |
| --- | --- | --- | --- |
| SQL для аналитика | 121 | 20 | Запрос выполняется в SQLite (sql.js) |
| Аналитика как профессия | 79 | 20 | Выбор варианта с разбором (`predict`) и расстановка шагов рассуждения по порядку (`order`) |
| Модель данных и BI | 77 | 19 | Выбор варианта с разбором (`predict`), дописывание формулы DAX со сверкой текстом (`fill`) и расстановка шагов по порядку (`order`) |
| pandas для аналитика | 65 | 14 | Код выполняется в Python (Pyodide) |

**Режим истории — рабочая неделя аналитика.** Семь недель по шесть дней,
сорок два дня, в трёх частях: первые две недели стажёра (SELECT, фильтры,
соединения), три недели аналитика (LEFT JOIN, пустые значения, CASE, CTE,
определение метрики) и две недели на pandas. Утром заказчик приходит
с вопросом, вечером вы приносите находку. Приёмы вводятся ровно там, где
дело в них упирается, а не по программе курса, и неделя умеет менять трек
между днями: одна ходит между профессией и SQL, последние две идут на
pandas. Часть 1 кончается названным результатом и картой данных; кто уже
знает SELECT и JOIN, может войти сразу в часть 2, а знающий SQL и желающий
pandas — в часть 3. Помечен в интерфейсе как «Бета». Рядом с ним идёт
отдельная сюжетная линия через каждый трек, прозой закрытая для всех
четырёх.
...
- **Песочница** — свободный режим на тех же данных: любой запрос, без задания
  и без подсказок, со списком вопросов рядом, если спросить нечего, и с
  шестнадцатью готовыми запросами под частые задачи — каждый на SQL и на
  pandas, каждый называет, что в нём меняют под свой вопрос;
...
## До какого уровня доводит

Пять глаголов — то, к чему ведёт семинедельная кампания:

- **Ask** — превратить «сделай мне дашборд» в вопрос с определённой
  метрикой (трек «Аналитика как профессия»).
- **Explore** — прочитать таблицы, гранулярность, ключи и что упущено,
  прежде чем доверять числу (трек SQL, экран «Данные»).
- **Analyse** — проверить гипотезу через SQL, pandas или DAX.
- **Explain** — что на самом деле показывают данные против того, что
  вы предполагали.
- **Recommend** — выбрать между действиями и сказать, почему остальные
  не годятся.
...
| `npm run verify` | Все 16 гейтов: датасет, контент, планировщик повторений, слияние прогресса, правила графика, раскладка схемы, английская проза, сверка текстом для DAX, миграция хранилища, сверка двух исполнителей, расписание push-напоминаний, ширина колонок в CSS, сюжетная линия, лестница режима истории жанр блоков в карточке приёма и числа README против контент-паков |
