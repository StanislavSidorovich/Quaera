...
**Четыре трека — 332 задания и 73 навыка.** Наполнены целиком, вычитаны
и переведены на английский: интерфейс, задания, карточки приёмов.

| Трек | Заданий | Навыков | Как проверяется ответ |
| --- | --- | --- | --- |
| SQL для аналитика | 115 | 20 | Запрос выполняется в SQLite (sql.js) |
...
**Режим истории — рабочая неделя аналитика.** Пять недель по шесть дней,
тридцать дней: утром заказчик приходит с вопросом, вечером вы приносите
...
между профессией и SQL, четвёртая и пятая идут на pandas. Помечен в
интерфейсе как ранний прототип. Рядом с ним идёт отдельная сюжетная линия
через каждый трек, прозой закрытая для всех четырёх.
...
Пять глаголов — то, к чему ведёт пятинедельная кампания:
