...
1. **Один связный датасет вместо учебных табличек.** Вымышленный дистрибьютор
   FMCG и OTC-фармы «Нордвинд Трейд», звезда из 12 таблиц, 156 тысяч строк
...
- **Показать сами данные, а не только схему.** Сейчас шторка со схемой
  перечисляет колонки и гранулярность, но не показывает ни одной строки
  и ни одной связи между таблицами — при том что `JOIN` нужен в 25 заданиях
  из 60 в SQL. Нужны несколько строк примера на таблицу и явные ключи.
- **Сохранение набранного при возврате на шаг занятия.** Вернуться к
  предыдущему шагу можно, но состояние редактора при этом обнуляется.
- **График результата** — переключатель «таблица / график» там, где
  результат имеет форму «категория + число» или «дата + число».
